...
This is the single source of truth for the 18-class Tianchi
"smartdiagnosisofclothflaw" taxonomy used by the trained RT-DETR model (two
visually-indistinguishable pairs merged 2026-09-23: surface_mark->broken_warp,
coarse_weft->weft_shrink).

The ``DefectType`` member order matches the model's class-id order (0..17), so
...
class DefectType(str, Enum):
    """Tianchi cloth-defect categories (model class id 0..17) plus OTHER."""
...
# The 18 model classes (id 0..17). Excludes the OTHER fallback so the model
# head width stays 18.
...
# Predicted class -> visually-confusable higher-impact classes. After the
# 2026-09-23 merge (surface_mark->broken_warp, coarse_weft->weft_shrink) the only
# minor-lookalike-of-critical case (surface_mark ~ broken_warp) is gone, so this
# map is empty. Kept so effective_alert_severity() stays a valid no-op and the
# escalation path remains auditable.
